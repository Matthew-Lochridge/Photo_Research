classdef class_SpectraScanPR655 < handle
    ...
    methods
        ...
        function set_timeout(camera, timeout)
            set(camera.serial_device, 'Timeout', timeout);
        end
        ...
        function status = connect(camera)
            write(camera.serial_device, 'P', 'char');
            write(camera.serial_device, 'H', 'char');
            write(camera.serial_device, 'O', 'char');
            write(camera.serial_device, 'T', 'char');
            write(camera.serial_device, 'O', 'char');
            status = read(camera.serial_device, 12, 'char');
        end
        ...
        function disconnect(camera)
            write(camera.serial_device, 'Q', 'char');
        end
        ...
        function status = update_status(camera)
            camera.status_code = str2double(writeread(camera.serial_device, 'I'));
        ...
        % Syntax: C[CR] 
        % Response: None 
        function status = clear_error(camera)
            camera.status_code = str2double(writeread(camera.serial_device, 'C'));
            status = camera.read_code('Error', camera.status_code);
        end
        ...
        function [source_frequency, status] = measure_source_frequency(camera) 
            response = str2double(split(writeread(camera.serial_device, 'F'), ','));
        ...
        function status = write_to_disk(camera)
            camera.status_code = str2double(writeread(camera.serial_device, 'D0'));
            status = camera.read_code('Error', camera.status_code);
        end
        ...
        function status = update_specs(camera)
            ...
            camera.addon2 = camera.read_code('Addon', response(4));
        ...
        % Syntax: E[CR] 
        % Response: None 
        function status = toggle_echo(camera)
            if strcmpi(camera.echo_mode, 'Disabled')
                camera.echo_mode = 'Enabled';
            elseif strcmpi(camera.echo_mode, 'Enabled')
                camera.echo_mode = 'Disabled';
            end
            camera.status_code = str2double(writeread(camera.serial_device, 'E'));
            status = camera.read_code('Error', camera.status_code);
        end
        ...
        function status = select_addon1(camera, description)
            camera.status_code = str2double(writeread(camera.serial_device, append('SA', num2str(camera.get_code('Addon', description)))));
            status = camera.read_code('Error', camera.status_code);
        end

        function status = select_addon2(camera, description)
            camera.status_code = str2double(writeread(camera.serial_device, append('SB', num2str(camera.get_code('Addon', description)))));
            status = camera.read_code('Error', camera.status_code);
        end

        function status = select_addon3(camera, description)
            camera.status_code = str2double(writeread(camera.serial_device, append('SC', num2str(camera.get_code('Addon', description)))));
            status = camera.read_code('Error', camera.status_code);
        end
        ...
        % Response: 0000[CR][LF] If all OK, else NNNN[CR][LF] (NNNN = Error code) 
        function status = set_source_frequency(camera, source_frequency)
            camera.status_code = str2double(writeread(camera.serial_device, append('SK', num2str(source_frequency))));
            status = camera.read_code('Error', camera.status_code);
        end
        ...
        % Response: 0000[CR][LF] If all OK, else NNNN[CR][LF] (NNNN = Error code) 
        function status = set_cycles_to_average(camera, num_cycles)
            camera.status_code = str2double(writeread(camera.serial_device, append('SN', num2str(num_cycles))));
            status = camera.read_code('Error', camera.status_code);
        end
        ...
        %        10 = 10 deg. 
        % Response: 0000[CR][LF] If all OK, else NNNN[CR][LF] (NNNN = Error code) 
        function status = select_CIE_observer(camera, description)
            camera.status_code = str2double(writeread(camera.serial_device, append('SO', num2str(camera.get_code('CIE Observer', description)))));
            status = camera.read_code('Error', camera.status_code);
        end
        ...
        function status = select_primary_lens(camera, description)
            camera.status_code = str2double(writeread(camera.serial_device, append('SP', num2str(camera.get_code('Primary', description)))));
            status = camera.read_code('Error', camera.status_code);
        end
        ...
        %        3 = User Frequency 
        % Response: 0000[CR][LF] If all OK, else NNNN[CR][LF] (NNNN = Error code) 
        function status = select_sync_mode(camera, description)
            camera.status_code = str2double(writeread(camera.serial_device, append('SQ', num2str(camera.get_code('Sync Mode', description)))));
            status = camera.read_code('Error', camera.status_code);
        end
        ...
        %        1 = Metric (SI) 
        % Response: 0000[CR][LF] If all OK, else NNNN[CR][LF] (NNNN = Error code) 
        function status = select_unit_system(camera, description)
            camera.status_code = str2double(writeread(camera.serial_device, append('SU', num2str(camera.get_code('Unit System', description)))));
            status = camera.read_code('Error', camera.status_code);
        end
        ...
        % Response: 0000[CR][LF] If all OK, else NNNN[CR][LF] (NNNN = Error code) 
        function status = select_shutter_mode(camera, description)
        ...
        function status = enable_reset(camera)
            camera.status_code = str2double(writeread(camera.serial_device, 'ZEnableReset', 'char'));
            status = camera.read_code('Error', camera.status_code);
        end

        function reset_preferences(camera)
            write(camera.serial_device, 'ZResetPreferences', 'char');
        end

        function reset_setup(camera)
            write(camera.serial_device, 'ZResetSetup', 'char');
        end

        function code = get_code(camera, field, description)
            code = []; % default output
            switch field
                case 'Primary'
                    switch description
        ...
        function description = read_code(camera, field, code)
            ...
            switch field
                ...
                case 'Unit'
                    switch code
                        case 111 % Luminance
                            if strcmpi(camera.unit_system, 'Imperial')
                                description = 'fL';
                            elseif strcmpi(camera.unit_system, 'SI')
                                description = 'cd/m^2';
                            end
                        case 11 % Radiance
